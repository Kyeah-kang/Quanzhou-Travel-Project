...
- **关键事实必须抽原文复核，不采信任何转述。**（2026-09-13 立规，源自两个真实反例）
  - **反例一（"读不到"≠"不存在"）**：内置网页抓取工具对本项目多数 `gov.cn` 域名返回「无法验证域名安全」而拒取，**看起来像"这处没有来源"，其实只是工具被拦**。改用 `httpx` / `curl` 直连即可取到原文。**不得因抓取工具报错就判定来源不存在、更不能据此填「查不到」。**
  - **反例二（转述必须回原文验证）**：事实核查返回的逐字引用中，有一条声称「官方申遗站列出六大关键要素名称」，抽原文复核时**该页找不到那六个词**。5 条引文里 4 条逐字命中、1 条不成立——**90% 的命中率在史实项目里等于不能采信**。
  - **复核办法**：抓原页 → 去 HTML 标签 → 搜关键词 → 打印命中处上下文 → 逐字比对。**尺寸 / 年代 / 数量 / 人名一律按此办理**，无论来源看起来多权威。
- 同一事实出现多个来源且数字不一致时，**全部保留为异说，不得只取一个装作没有分歧**；记录格式见 `docs/corpus-spec.md` §5。
