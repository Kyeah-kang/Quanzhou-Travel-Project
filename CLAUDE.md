...
### 每天的固定闭环
1. 按课表确认当天模块：范围、分层落点、验收标准（供 Codex 实现；若学生已让 Codex 写好，则跳过实现、直接评审）。
2. **带学生 review 当天代码**：逐段讲解“这段在做什么、逻辑/含义/为什么这么写”，对应实现代码逐行走查（老师讲解格式见 §8）。
3. 运行/测试验证（uvicorn/pytest/页面）；指出问题但不越权大改；必要时给出“请 Codex 修改 X”的清单。
4. 更新当日笔记 `.plan/daily/` 与课表状态。
5. 次日先做**费曼复盘**（学生复述昨日模块，你答疑），再进新模块。
6. 你自己亲自写代码时，同样遵守 `AGENTS.md` 的注释与提交规范。
